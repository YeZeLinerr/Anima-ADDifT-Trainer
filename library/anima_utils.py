...
def load_anima_vae(vae_path: str, dtype: torch.dtype = torch.float32, device: str = "cpu"):
    ...
    # Load state dict
    if vae_path.endswith(('.safetensors', '.sft')):
        vae_sd = load_file(vae_path, device='cpu')
    else:
        vae_sd = torch.load(vae_path, map_location='cpu', weights_only=True)

    required_vae_keys = ("encoder.conv1.weight", "decoder.conv1.weight", "conv1.weight", "conv2.weight")
    missing_required = [key for key in required_vae_keys if key not in vae_sd]
    if missing_required:
        sample_keys = list(vae_sd.keys())[:8]
        raise ValueError(
            "VAE state_dict does not match the Anima/WanVAE architecture. "
            f"Missing required keys: {missing_required}. "
            f"First checkpoint keys: {sample_keys}. "
            "Use the Anima/Qwen Image VAE, not a standard autoencoder VAE such as ae.sft."
        )
