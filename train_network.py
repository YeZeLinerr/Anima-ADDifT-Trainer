...
class NetworkTrainer:
    ...
    def train(self, args):
        ...
        if args.dataset_class is None:
            support_paired_conditioning = bool(getattr(args, "paired_difference_mode", False))
            blueprint_generator = BlueprintGenerator(
                ConfigSanitizer(True, True, args.masked_loss or support_paired_conditioning, True)
            )
            if use_user_config:
        ...
        # assert_extra_args may change DataLoader-related settings. Build the
        # collator afterwards so its dataset ownership matches num_workers.
